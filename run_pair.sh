...
run_tests() {
    ...
    flight_recording_option="-XX:+FlightRecorder -XX:StartFlightRecording=filename=flight_$iteration.jfr,settings=$jfc_file -XX:FlightRecorderOptions=stackdepth=1024"
    ...
    /home/runtimeprofiler/apache-maven/bin/mvn test -DargLine="$flight_recording_option $gc_option" -DjfrPath="./flight_$iteration.jfr" $MVN_OPTS
    python3 "$insert_jvm_args_in_pom_path" --dir "$project_root_dir" --restore

    cp "flight_$iteration.jfr" "$dest"
    cp gc.log "$dest"
